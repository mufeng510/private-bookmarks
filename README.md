...
| 设置跟随账号 | 服务器地址、Sync Token、同步周期等设置保存在浏览器账号的同步存储（chrome.storage.sync）：Chrome 走 Google 账号、Edge 走 Microsoft 账号，登录同一账号的新设备 / 重装后的系统自动恢复全部配置 |
| 自定义 Client ID | 扩展设置页可把随机 ID 改成好记的名字（如 `chrome-desktop`）。Client ID 仅保存在本机（不随账号同步），**重装系统 / 重装扩展后填回同一个 ID，同步数据就延续同一台"设备"** |
...
2. 重装后安装扩展并登录同一浏览器账号：服务器地址、Sync Token 等设置**自动恢复**（账号同步）
3. Client ID 是本机标识，不会自动恢复：填回原来的 ID 即延续该设备数据；直接用新生成的 ID 则视为新设备（旧数据可在网站设置页「删除数据」清理）
